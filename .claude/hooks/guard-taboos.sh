...
# KEYPRIV, widened: its boundary also lets "-cert." pass, so a
# certificate beside its key (ssh_host_ed25519_key-cert.pub) is
# public like a .pub; any other hyphen suffix
# (ssh_host_rsa_key-old, id_ed25519-work) is still a private key.
# ERE has no lookahead, so "-cert." is excluded letter by letter.
# Set here rather than in the line above only while #13 rewrites
# the definitions next to it (heinzel#27); fold it in afterwards.
